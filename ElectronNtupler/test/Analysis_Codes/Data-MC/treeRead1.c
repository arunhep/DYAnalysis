...
//  char *file1[9] = {"DYEE", "DYTT", "WJetsToLNu", "TTbar", "diBoson_WW", "diBoson_WZ", "diBoson_ZZ", "Single_antiTop", "SingleTop"};

 char *file1[9] = {"DYEE_M10to3000", "DYTauTau_M10to3000", "dieleBkg_1", "dieleBkg_2", "dieleBkg_3", "dieleBkg_4", "dieleBkg_5", "dieleBkg_6", "dieleBkg_7"};
  ...
  TH1F *dieleMass_Peak[9]; TH1F *dielePT[9]; TH1F *dieleEta[9]; TH1F *dieleRap[9]; TH1F *dielePhi[9]; 
  ...
  for(int i=0;i<9;i++){
    ...
    //sprintf(name, "/home/ridhi/Work/Analysis/Tuples/13TeV/DYSpectrum/dzMask/%s.root", file1[i]);
    TFile *f = TFile::Open(name);
    
    //sprintf(name, "/home/ridhi/Work/Analysis/DYSpectrum/%s_new.root", file1[i]);
    sprintf(name, "%s_new.root", file1[i]);
    //sprintf(name, "dzMask/%s_new.root", file1[i]);
    ...
    ele1PT[i]  = new TH1F("ele1PT", "ele1PT" , 100, 0, 700);
    ele2PT[i]  = new TH1F("ele2PT", "ele2PT" , 100, 0, 700);
    elePT[i]   = new TH1F("elePT", "elePT" , 100, 0, 700);
    ...
    /*dieleMass[i]    = new TH1F("dieleMass", "dieleMass", 200, 80, 100);
    dieleMass_BB[i] = new TH1F("dieleMass_BB", "dieleMass_BB", 200, 80, 100);
    dieleMass_BE[i] = new TH1F("dieleMass_BE", "dieleMass_BE", 200, 80, 100);
    dieleMass_EE[i] = new TH1F("dieleMass_EE", "dieleMass_EE", 200, 80, 100);*/

    dieleMass_Peak[i] = new TH1F("dieleMass_Peak", "dieleMass_Peak", 30, 60, 120);
    /*dielePT[i]   = new TH1F("dielePT", "dielePT" , 100, 0, 700);
    ...
    dieleMass_Peak[i]->Sumw2(); //dielePT[i]->Sumw2(); dieleEta[i]->Sumw2(); dieleRap[i]->Sumw2(); dielePhi[i]->Sumw2();
    nPV[i]->Sumw2();
    nPV_wt[i]->Sumw2();
    ...
    for(Long64_t j=0;j<entries;j++){
      ...
      double scale = lumiWeight*genWeight*2320;
      ...
      dieleMass[i]->Fill(corrMass,scale*scale1);
      nPV[i]->Fill(primVtx,scale);
      nPV_wt[i]->Fill(primVtx,scale*scale1);

      /*if(BB) dieleMass_BB[i]->Fill(corrMass,scale);
      if(BE) dieleMass_BE[i]->Fill(corrMass,scale);
      if(EE) dieleMass_EE[i]->Fill(corrMass,scale);*/

      dieleMass_Peak[i]->Fill(corrMass,scale*scale1);
      /*dielePT[i]->Fill(ZPT,scale);
      dieleEta[i]->Fill(ZEta,scale);
      dieleRap[i]->Fill(ZRapidity,scale);
      dielePhi[i]->Fill(ZPhi,scale);*/
    }

    //sprintf(histname, "dieleMass_%s", file1[i]);
    //dieleMass[i]->SetName(histname);
