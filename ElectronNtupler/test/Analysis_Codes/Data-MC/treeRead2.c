{
  char *file1[1] = {"SingleElectron_ScaleCorr"};
  ...
  TH1F *dieleMass_Peak[1]; TH1F *dielePT[1]; TH1F *dieleEta[1]; TH1F *dieleRap[1]; TH1F *dielePhi[1]; 
 TH1F *nPV[1];
  TFile *file2[1];
...
//cout << Ele1Phi << Ele2Phi << endl;
      nPV[i]->Fill(primVtx);
      dieleMass[i]->Fill(ZMass);
      /*if(BB) dieleMass_BB[i]->Fill(ZMassCorr);
      if(BE) dieleMass_BE[i]->Fill(ZMassCorr);
      if(EE) dieleMass_EE[i]->Fill(ZMassCorr);*/
