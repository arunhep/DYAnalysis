{
  ...
  TFile *f1 = TFile::Open("Plots_MediumID_Calibrated/SingleElectron_UnScaleCorr_new.root"); //Data
  TFile *f2 = TFile::Open("Plots_MediumID_Calibrated/DYEE_M10to3000_new.root"); //DYEE
  ...
  for(int i=0;i<10;i++){
    ...
    hStack1->SetMinimum(0);
    hStack1->SetMaximum(10000);
    ...
    sprintf(name, "Plots_MediumID_Calibrated/Plots/%s.png", histo[i]);
    c1->SaveAs(name);
    delete c1;
  } //loop
