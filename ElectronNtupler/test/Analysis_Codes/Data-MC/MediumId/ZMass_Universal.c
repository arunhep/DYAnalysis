{
  ...
  TFile *f1 = TFile::Open("Plots_MediumID/SingleElectron_UnScaleCorr_new.root"); //Data
  TFile *f2 = TFile::Open("Plots_MediumID/DYEE_M10to3000_new.root"); //DYEE
  ...
  for(int i=0;i<10;i++){
    ...
    sprintf(name, "Plots_MediumID/Plots/%s.png", histo[i]);
    c1->SaveAs(name);
    delete c1;
  } //loop
